...
#define KBD_TEST_PASS 0x55
#define KBD_TEST_FAIL 0xFC

#define KBD_CMD_DISABLE_PORT_1  0xAD
#define KBD_CMD_ENABLE_PORT_1   0xAE
#define KBD_CMD_DISABLE_PORT_2  0xA7
#define KBD_CMD_ENABLE_PORT_2   0xA8
...
typedef void (*pfn_keyboard_callback)(int scancode);

/**
 * Attempts to initialize the first PS/2 Keyboard found
 * @returns
 * @true if succeeded
 * @false if failed
 */
...
/**
 * Sets the `enabled` flag to true, therefore allowing input
 */
void ps2_keyboard_enable();

/**
 * Sets the `enabled` flag to false, therefore disallowing input
 */
void ps2_keyboard_disable();

/**
 * Queries the state of the `enabled` flag
 */
bool ps2_keyboard_is_enabled();

/**
 * Reads the scancode as a uint8_t
 */
uint8_t ps2_keyboard_read_scancode();

/**
 * Processes the scancode read from `keyboard_read_scancode`
 * and returns the associated character
 */
char ps2_keyboard_read_key();

/**
 * Registers a keybord callback to process unhandled scancodes
 * Note: only one handler can be active at a time,
 * Callback must take a single `int` argument which represents the scancode
 * received, and return void
 */
void ps2_keyboard_register_callback(pfn_keyboard_callback callback);

/**
 * The default procedure called in `keyboard_read_key` which returns the
 * character associated to the scancode provided
 */
